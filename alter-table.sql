ALTER TABLE `salinet_tur`.`tbl_turistas` 
ADD COLUMN `cpf` INT(11) NULL AFTER `nome`,
ADD COLUMN `data_de_nascimento` VARCHAR(10) NULL AFTER `cpf`,
ADD COLUMN `rg` VARCHAR(13) NULL AFTER `data_de_nascimento`,
ADD COLUMN `orgao_expedidor_rg` VARCHAR(6) NULL AFTER `rg`,
ADD COLUMN `sobrenome` VARCHAR(40) NULL AFTER `orgao_expedidor_rg`,
ADD COLUMN `profissao` VARCHAR(30) NULL AFTER `sobrenome`,
